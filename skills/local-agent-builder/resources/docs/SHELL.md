...
Giving autonomous agents access to the local shell introduces monumental security risks. The `local-agent-builder` scaffold ships with an optional, highly constrained `run_shell_command` tool located in `src/tools/shell.py`.
...
1. **OS Constraints**: Anthropic SRT runs natively over Linux (`bwrap`) and macOS (`sandbox-exec`). **It strictly does not support Windows natively**. If you are on Windows, you must disable the sandbox.
